"""
Interface for Candapter interfaces (win32/linux).
"""
...
try:
    ...
    logger.warning(
        "You won't be able to use the candapter can backend without "
        "the serial module installed!"
    )
    serial = None


class candapterBus(BusABC):
    """
    Candapter interface
    """
    ...
    def _write(self, string: str) -> None:
        with error_check("Could not write to serial device"):
            logger.debug(f"{string.encode() + self.LINE_TERMINATOR}")
        ...
        with error_check("Could not read from serial device"):
            while True:
                ...
                for _ in range(max(1, in_waiting)):
                    ...
                    if new_byte in (self._ERROR, self._OK, self.LINE_TERMINATOR):
    ...
    def get_version(
        ...
        """Get HW and SW version of the candapter interface.
    ...
    def get_serial_number(self, timeout: Optional[float]) -> Optional[str]:
        """Get serial number of the candapter interface.
